...
function VehiclesContent() {
  ...
  const fetchData = async () => {
    try {
      ...
      setVehiculos(vData);
      setClients(cData.filter((u: any) => u.role === 'CLIENTE'));
    } catch (err) {
      console.error(err);
    } finally {
      setIsLoading(false);
  ...
  const handleToggleStatus = async (vehicle: Vehicle) => {
    ...
    try {
      const response = await fetch(`/api/vehiculos/${vehicle.id}`, {
        method: 'PATCH',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEditing ? `/api/vehiculos/${selectedVehicleId}` : '/api/vehiculos';
      ...
      if (!response.ok) throw new Error(data.error || 'Error al procesar vehículo');
